...
## Personas and jobs
- Primary personas: phone readers opening local TXT/PDF/EPUB/CBZ/image files; tablet/foldable readers using longer sessions
...
## Interaction model
- Pointer input observers never consume events unless they own the gesture outcome. Reader multi-touch owns the Initial pass ahead of page navigation, keeps ownership until every pointer lifts, and fixed visual-page one-finger pan only takes over while zoom > 1.
...
- Supported visual-file constraint: CBZ (ZIP) and JPG/JPEG/PNG/WebP/GIF/BMP are supported; CBR/RAR is excluded until a maintained KMP extractor is justified.
