...
**페이지 넘김.** 열 가지 — 없음, 슬라이드, 페이드, 스크롤, 플루이드 페이지, 컬 페이지, 3D 컬,
원형 전환, 무비 캐러셀, 페이지 플립. 컬, 3D 컬과 페이지 플립은 손가락을 따라 움직이다가 놓는
지점에서 자리를 잡고, 펼침 화면에서는 전체를 밀어내지 않고 한 장만 책등에서 접습니다. 페이지
플립은 앞·뒷면 조명과 아래 페이지에 드리우는 그림자를 함께 표현합니다.
